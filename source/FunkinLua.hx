// I'll be trying to Pull the Lua code from Psych lmao
// wish me luck
// https://cdn.discordapp.com/attachments/848938574254440468/965553957374156820/wassup_dataminer.gif
import llua.Lua;
import llua.LuaL;
import llua.State;
import llua.Convert;
...
class FunkinLua {
	...
	function getGroupStuff(leArray:Dynamic, variable:String) {
		var killMe:Array<String> = variable.split('.');
		if(killMe.length > 1) {
			var coverMeInPiss:Dynamic = Reflect.getProperty(leArray, killMe[0]); /*so tasty variable name ngl*/
			for (i in 1...killMe.length-1) {
				coverMeInPiss = Reflect.getProperty(coverMeInPiss, killMe[i]);
			}
			return Reflect.getProperty(coverMeInPiss, killMe[killMe.length-1]);
